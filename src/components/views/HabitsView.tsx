...
export const HabitsView: React.FC<HabitsViewProps> = ({
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-150">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-2xl bg-white border border-slate-200 shadow-sm">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-bold text-slate-900">Daily Discipline & Habit Matrix</h2>
          </div>
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-orange-600 hover:bg-orange-700 text-white text-xs font-semibold shadow-sm transition-all self-start sm:self-auto"
        >
      ...
      {isAdding && (
        <form onSubmit={handleCreateHabit} className="p-4 rounded-2xl bg-white border border-orange-200 shadow-sm space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-orange-700">Create New Habit Routine</span>
            <button
              type="button"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <input
              ...
              className="sm:col-span-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-orange-500"
            />

            <select
              value={newWorkspace}
              onChange={(e) => setNewWorkspace(e.target.value as any)}
              className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-800 focus:outline-none focus:border-orange-500 font-medium"
            >
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button
              type="submit"
              className="px-4 py-1.5 bg-orange-600 hover:bg-orange-700 text-white rounded-xl text-xs font-semibold shadow-sm"
            >
              Add Routine
            </button>
          </div>
      ...
      {habits.length === 0 ? (
        <div className="py-16 text-center border border-dashed border-slate-200 rounded-2xl p-6 bg-white space-y-3 shadow-sm">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-900">No Habit Routines Tracked Yet</h3>
          ...
          <button
            onClick={() => setIsAdding(true)}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-orange-600 hover:bg-orange-700 text-white text-xs font-semibold shadow-sm transition-all"
          >
            <Plus className="w-4 h-4" />
        ...
        <>
          ...
          <div className="md:hidden space-y-3">
            {habits.map((habit) => (
              <div key={habit.id} className="p-4 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="text-xs font-bold text-slate-900">{habit.title}</div>
                      <div className="text-[10px] text-slate-400 font-mono">{habit.category}</div>
                    </div>
                  </div>

                  <div className="flex items-center gap-2">
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-orange-50 text-orange-700 border border-orange-200 text-[10px] font-mono font-bold">
                      🔥 {habit.streak}d
                    </span>
                    <button
                    ...
                    return (
                      <button
                        ...
                        className={`flex flex-col items-center justify-center py-2 rounded-xl border transition-all text-center ${
                          d.isToday ? 'ring-2 ring-indigo-400/50' : ''
                        } ${
                          isCompleted
                      ...
                      >
                        <span className={`text-[9px] font-mono ${isCompleted ? 'text-emerald-100' : 'text-slate-400'}`}>
                          {d.dayName}
                        </span>
                        <span className={`text-xs font-bold font-mono mt-0.5 ${isCompleted ? 'text-white' : 'text-slate-700'}`}>
                          {d.dayNum}
                        </span>
          ...
          <div className="hidden md:block rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50 text-slate-500 font-mono text-[10px] tracking-wider uppercase">
                    <th className="py-3 px-4 min-w-[220px]">Routine & Sphere</th>
                    <th className="py-3 px-3 w-28 text-center">Streak</th>
                    {days.map((d) => (
                      <th key={d.dateStr} className={`py-3 px-2 text-center w-14 ${d.isToday ? 'text-indigo-600 font-bold bg-indigo-50/60' : ''}`}>
                        <div>{d.dayName}</div>
                        <div className="text-[11px] text-slate-800">{d.dayNum}</div>
                      </th>
                    ))}
                    ...
                    return (
                      <tr key={habit.id} className="hover:bg-slate-50 transition-colors group">
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <div className="text-[10px] text-slate-500 font-mono">{habit.category}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="py-3 px-3 text-center">
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-orange-50 text-orange-700 border border-orange-200 text-[11px] font-mono font-bold">
                            🔥 {habit.streak}d
                          </span>
                        </td>
                        ...
                        {days.map((d) => {
                          ...
                          return (
                            <td
                              key={d.dateStr}
                              className={`py-3 px-2 text-center ${d.isToday ? 'bg-indigo-50/40' : ''}`}
                            >
                              <button
                                onClick={() => onToggleDate(habit.id, d.dateStr)}
                                className={`w-7 h-7 rounded-lg border transition-all inline-flex items-center justify-center mx-auto ${
                                  isCompleted
                                    ...
                                    : 'border-slate-200 bg-slate-50 hover:border-slate-300 text-transparent hover:text-slate-400'
                                }`}
                              >
