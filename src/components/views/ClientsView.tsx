    ...
    <div className="space-y-6 animate-in fade-in duration-150">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-2xl bg-white border border-neutral-200 shadow-sm">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-bold text-neutral-900">Client Portfolio & Contracts</h2>
          </div>
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-zinc-700 hover:bg-zinc-800 text-white text-xs font-semibold shadow-sm transition-all self-start sm:self-auto"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-4 rounded-xl bg-white border border-neutral-200 shadow-sm">
          <div className="text-xs text-neutral-500 font-mono uppercase">Total Accounts</div>
          <div className="text-xl font-bold text-neutral-900 mt-1 font-mono">{clients.length}</div>
        </div>

        <div className="p-4 rounded-xl bg-white border border-neutral-200 shadow-sm">
          <div className="text-xs text-neutral-500 font-mono uppercase">Active Retainers</div>
          <div className="text-xl font-bold text-stone-600 mt-1 font-mono">{activeClients.length}</div>
        </div>

        <div className="p-4 rounded-xl bg-white border border-neutral-200 shadow-sm">
          <div className="text-xs text-neutral-500 font-mono uppercase">Active Deliverables</div>
          <div className="text-xl font-bold text-zinc-700 mt-1 font-mono">
            {clientTasks.filter(t => t.status !== 'done').length}
          </div>
        </div>

        <div className="p-4 rounded-xl bg-white border border-neutral-200 shadow-sm">
          <div className="text-xs text-neutral-500 font-mono uppercase">Delivered Milestones</div>
      ...
      {clients.length === 0 ? (
        <div className="py-16 text-center border border-dashed border-neutral-200 rounded-2xl p-6 bg-white space-y-3 shadow-sm">
          <div className="w-12 h-12 rounded-2xl bg-zinc-100 text-zinc-700 border border-zinc-300 flex items-center justify-center mx-auto">
            <Building2 className="w-6 h-6" />
          </div>
          <div>
            <h3 className="text-sm font-bold text-neutral-900">No Client Accounts Registered Yet</h3>
          ...
          <button
            onClick={() => onOpenClientModal()}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-zinc-700 hover:bg-zinc-800 text-white text-xs font-semibold shadow-sm transition-all"
          >
            <Plus className="w-4 h-4" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {clients.map((client) => {
            ...
            return (
              <div
                key={client.id}
                className="p-5 rounded-2xl bg-white border border-neutral-200 hover:border-zinc-500 transition-all flex flex-col justify-between group space-y-4 shadow-sm hover:shadow-md"
              >
                <div>
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2.5">
                      <div
                        ...
                        style={{ backgroundColor: client.color || '#171717' }}
                      >
                        {client.company.slice(0, 2).toUpperCase()}
                      </div>
                      <div>
                        <h3 className="text-sm font-bold text-neutral-900 group-hover:text-zinc-800 transition-colors">
                          {client.company}
                        </h3>
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-3 text-[11px] font-mono">
                    ...
                    <span className="px-2 py-0.5 rounded bg-neutral-50 text-neutral-700 border border-neutral-200 flex items-center gap-1 font-medium">
                      <DollarSign className="w-3 h-3 text-zinc-700" />
                      {client.rate}
                    </span>

                    {client.totalBudget && (
                      <span className="px-2 py-0.5 rounded bg-neutral-50 text-neutral-600 border border-neutral-200">
                  ...
                  <div className="mt-3 space-y-1.5">
                    {associatedTasks.slice(0, 2).map((t) => (
                      <div
                        ...
                        className="flex items-center justify-between p-2 rounded-lg bg-neutral-50 hover:bg-neutral-100 text-xs cursor-pointer transition-colors border border-neutral-100"
                      >
                        <div className="flex items-center gap-2 truncate">
                          <button
                            ...
                            className={`w-3.5 h-3.5 rounded border flex items-center justify-center flex-shrink-0 ${
                              t.status === 'done' ? 'bg-stone-500 border-stone-500 text-white' : 'border-neutral-300 hover:border-stone-500'
                            }`}
                          >
                            {t.status === 'done' && <CheckCircle2 className="w-2.5 h-2.5" />}
                          </button>
                          <span className={`truncate font-medium ${t.status === 'done' ? 'line-through text-neutral-400' : 'text-neutral-800'}`}>
                            {t.title}
                          </span>
                        </div>
                ...
                <div className="pt-3 border-t border-neutral-100">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-1.5 py-1.5 rounded-lg bg-zinc-100 hover:bg-zinc-200 text-zinc-900 text-xs font-semibold border border-zinc-300 transition-all"
                  >
